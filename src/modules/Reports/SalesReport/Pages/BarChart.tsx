...
type BarChart = { year: number; totalAmount: number };
  ...
  const yScale = useMemo(
    () =>
      scaleLinear<number>({
        range: [yMax, 0],
        round: true,
        domain: [0, Math.max(...data.map((d) => d.totalAmount))],
      }),
    [data, yMax]
  );
    ...
    <Fragment>
      <svg ref={containerRef} width={width} height={height}>
        ...
        <Group top={margin.bottom}>
          {data?.map(({ totalAmount, year }, index) => {
            const barWidth = xScale.bandwidth();
            const barHeight = yMax - (yScale(totalAmount) ?? 0);
