...
import {
  studentReportFilterSchema,
  FILTER_STUDENT_REPORTS,
  FilterStudentReportsArgs,
} from "modules/Reports/StudentReport";
import StackedBarChart from "modules/Reports/StudentReport/Pages/StackedBarChart";
...
const tableHeaders = [
  ...
  { name: "Month", id: "month" as const },
  { name: "Total New Student", id: "totalNewStudents" as const },
  {
    name: "Total Discontinued Student",
    id: "totalDiscontinuedStudents" as const,
  },
  // { name: "Total Active Student", id: "totalActiveStudents" as const },
  { name: "Total Graduated", id: "totalGraduated" as const },
];
...
const StudentReport = () => {
  ...
  const { control } = useFormWithZod({
    schema: studentReportFilterSchema,
    defaultValues: {
  ...
  const commonQueryArgs: FilterStudentReportsArgs = {
    fromDate,
    toDate,
  ...
  const { data, loading, fetchMore } = useQuery(FILTER_STUDENT_REPORTS, {
  ...
  const generateCSVHandler = () => {
    if (data?.filterStudentReports?.csvFilePath) {
      fileDownload(data?.filterStudentReports?.csvFilePath);
    } else {
  ...
  const rows =
    data?.filterStudentReports?.dataCollection?.map((studentReport, index) => {
      return {
        id: index + 1,
        year: studentReport?.year ?? 0,
        month: studentReport?.month,
        totalNewStudents: studentReport?.totalNewStudents ?? 0,
        totalDiscontinuedStudents:
          studentReport?.totalDiscontinuedStudents ?? 0,
        totalActiveStudents: studentReport?.totalActiveStudents ?? 0,
        totalGraduated: studentReport?.totalGraduatedStudents ?? 0,
      };
    }) ?? [];
  ...
  const onPrevOrNextHandler = (page: number) => {
    ...
    fetchMore({
      variables: queryArgs,
      updateQuery: (_, { fetchMoreResult: { filterStudentReports } }) => {
        return { filterStudentReports };
      },
    }).catch((error) => {
  ...
  const totalCount = data?.filterStudentReports?.totalNoOfRecords || 0;
    ...
    <div className="grid grid-cols-1 max-w-[904px] gap-6 py-2">
      ...
      >
        <div className="flex justify-between gap-6">
          <p className="text-[34px] font-normal font-sunbird">Student Report</p>
        ...
        <div className="shadow-gradient-elevation rounded-2xl p-4 space-y-4">
          <p className="text-2xl font-normal font-sunbird break-words line-clamp-2">
            {watchMasterFranchiseeOrFranchisee?.name ?? ""} Student Report [
            ...
            <StackedBarChart
              data={
                rows?.map((data) => {
                  return {
                    year: +data?.year,
                    // Active: data?.totalActiveStudents,
                    Discontinued: data?.totalDiscontinuedStudents,
                    Graduated: data?.totalGraduated,
                    ["New Student"]: data?.totalNewStudents,
                  };
                }) ?? []
              }
            />
