...
> = Omit<
  ...
  | "value"
  | "observe"
> & { args?: { filter?: ProductCategoriesFilterInput } };
      ...
      if (
        ...
        fetchMore({
          ...
          updateQuery: (
            ...
            if (!filterProductCategories) return prev;
            return {
              filterProductCategories:
                filterProductCategories?.edges &&
                filterProductCategories?.edges?.length > 0 &&
                prev?.filterProductCategories
                  ? prev?.filterProductCategories?.edges &&
                    prev?.filterProductCategories?.edges?.length > 0
                    ? {
                        edges: [
                          ...prev.filterProductCategories.edges,
                          ...filterProductCategories.edges,
                    ...
                    : {
                        edges: filterProductCategories?.edges,
                        pageInfo: filterProductCategories?.pageInfo,
                      }
                  : prev.filterProductCategories,
            };
  ...
  const options = useMemo(() => {
    return error?.message
      ...
      : data?.filterProductCategories?.edges &&
        data?.filterProductCategories?.edges?.length > 0
      ? data?.filterProductCategories?.edges
          ?.map((edges) => {
          ...
          .filter(Boolean)
      : [];
  }, [data?.filterProductCategories?.edges, error?.message]);
