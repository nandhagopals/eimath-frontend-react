  ...
  const options = useMemo(() => {
    return error?.message
      ...
      : data?.filterWorkbookInformation?.edges &&
        data?.filterWorkbookInformation?.edges?.length > 0
      ? data?.filterWorkbookInformation?.edges
          ?.map((edges) => {
          ...
          .filter(Boolean)
      : [];
  }, [data?.filterWorkbookInformation?.edges, error?.message]);
...
export default WorkbookManagementField;
